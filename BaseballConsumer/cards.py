...
FINAL_STATES = {'Game Over', 'Final', 'Game Over: Tied', 'Final: Tied',
...
def status_card(game, title, description, body="", *, accent=None, image_url=None):
    ...
    matchup = "{} {} @ {} {}".format(
        team_flair(away_id), game.get('away_name', ''),
        team_flair(home_id), game.get('home_name', '')).strip()
    if matchup:
        head.append("-# {}".format(matchup))
    heading = "\n".join(head) or "## Update"

    if home_id is not None:
        c.add_item(ui.Section(heading, accessory=ui.Thumbnail(media=team_logo_url(home_id))))
    else:
...
def final_card(game, our_team_id):
    ...
    c = ui.Container(accent_colour=accent)
    score_line = "## {} {} {} — {} {} {}".format(
        team_flair(away_id), game.get('away_name', 'Away'), away_score,
        home_score, game.get('home_name', 'Home'), team_flair(home_id))
    ...
    if gid:
        c.add_item(ui.ActionRow(
            ui.Button(label="Gameday", url="https://www.mlb.com/gameday/{}".format(gid))))
    return _layout(c)
...
def atbat_card(info):
    ...
    state = "### {} {}\n{}-{} count · {} {}".format(
        half, info['inning'], info['balls'], info['strikes'],
    ...
    if bases_image_path(info):
        c.add_item(ui.Section(body, accessory=ui.Thumbnail(media="attachment://" + BASES_ATTACHMENT)))
    else:
    ...
    if batter_id:
        c.add_item(ui.Section(score, accessory=ui.Thumbnail(media=player_headshot_url(batter_id))))
    else:
...
def end_of_inning_card(info):
    ...
    if pc:
        c.add_item(ui.TextDisplay("-# {} — {} pitches ({}-{})".format(pc[0], pc[3], pc[1], pc[2])))
...
def lineup_card(game_data):
    ...
    for side in ('away', 'home'):
        ...
        if team_id is not None:
            c.add_item(ui.Section(header, accessory=ui.Thumbnail(media=team_logo_url(team_id))))
        else:
...
def pitchcount_card(entries):
    ...
    for label, name, balls, strikes, total in entries:
        c.add_item(ui.TextDisplay("**{}** {} — {} pitches ({}-{})".format(
